...
# 함수명을 구버전과 다르게 재정의하여 캐시 원천 차단
def exec_stats_calculation(records_tuple, curr_date_filter=None):
    n = len(records_tuple)
    if n < 4: return None
    tot_a, tot_b = 0, 0
    a_win, b_win = 0, 0
    ...
    for i in range(3, n):
        ...
        if curr_date_filter and records_tuple[i][0] != curr_date_filter: continue
...
if st.session_state.show_bulk:
    ...
    # 1. 전체 누적 통계
    all_stat = exec_stats_calculation(records_tuple)
    ...
    # 2. 최근 3000개 누적 통계
    recent_stat = exec_stats_calculation(records_tuple[-3000:])
    st.markdown(f"**최근 {min(len(records), 3000)}개 누적 통계 (패스 회차 제외)**")
    if recent_stat:
    ...
    today_stat = exec_stats_calculation(records_tuple, curr_date_filter=curr_date)
