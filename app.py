...
st.set_page_config(page_title="키노사다리 2중 지울픽 검증기", page_icon="⚡", layout="centered")
...
def get_engine1_avoid(records_tuple, prev_failures):
    valid = [r[2] for r in records_tuple if r[2] in ALL_COMBOS]
    if len(valid) < 3: return '좌사'
    ...
    return f"{OPPOSITE_SINGLE_MAP[rec_combo[0]]}{OPPOSITE_SINGLE_MAP[rec_combo[1]]}"

# 🎯 [엔진 2: 신규 - 마르코프 단기 상태 이행 배제 엔진]
def get_engine2_avoid(records_tuple):
    ...
    # 상태 1: 동일 조합 2연속(우사-우사) 진입 시 동일 3연속 및 완전 대칭 반대는 배제
    if last == prev:
        avoid_pick = f"{OPPOSITE_SINGLE_MAP[last[0]]}{OPPOSITE_SINGLE_MAP[last[1]]}"
    # 상태 2: 교대 퐁당(우사-좌삼-우사) 진입 시 직전값 재출현 배제
    elif len(valid) >= 3 and valid[-3] == last:
        avoid_pick = last
    # 상태 3: 일반 꺾임 구간 시 직전과 동일 출발/동일 줄수 조합 배제
    else:
...
def analyze_double_avoid_system(records_tuple, prev_failures={'start': False, 'line': False, 'oe': False}, last_avoid_failed=False):
    ...
    avoid1 = get_engine1_avoid(records_tuple, prev_failures)
...
def calculate_stats(records_tuple, history_store, target_date=None):
    ...
    for i in range(3, n):
        ...
        prev_failures['start'] = (ITEM_MAP[res['avoid1']][0] == act_s)
        prev_failures['line'] = (ITEM_MAP[res['avoid1']][1] == act_l)
        prev_failures['oe'] = (ITEM_MAP[res['avoid1']][2] == act_o)
